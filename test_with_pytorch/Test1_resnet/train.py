...
net.fc = nn.Linear(inchannel, 11)
net.to(device)

loss_function = nn.CrossEntropyLoss()
optimizer = optim.Adam(net.parameters(), lr=0.0001)  #0.0005
...
for epoch in range(50):
    ...
    with torch.no_grad():
        ...
        acc_list.append(accurate_test)


print('Finished Training')

saving_list_l = np.array(loss_list)
np.save('losslist_res.npy', saving_list_l)
saving_list_a = np.array(acc_list)
np.save('acclist_res.npy', saving_list_a)
...
plt.ylabel("Loss")
plt.plot(loss_list)
plt.savefig(os.getcwd()+'res_loss.png')
plt.show()
...
plt.ylabel("Accuracy")
plt.plot(acc_list)
plt.savefig(os.getcwd()+'res_acc.png')
plt.show()